...
def draw_14x8():
    ...
    for i in indexes:
        ...
        for j, (nl, nh) in enumerate(selected_layer_head_pairs):
            ...
            res = axs[(i-3)*2, j].imshow(per_att,
                                       cmap=plt.cm.Blues,
                                       )
            axs[(i-3)*2, j].set_xlabel('Attentions From', fontsize=fs)
            axs[(i-3)*2, j].set_ylabel('Attentions To', fontsize=fs)
            # plt.colorbar(res, ax=axs[i, j])
            axs[(i-3)*2, j].title.set_text(
                f'Pos. Cases {i+1}\n Layer {nl+1} Head {nh+1}')
            axs[(i-3)*2, j].title.set_fontsize(fs)

            per_att = per_att[bgn_genp:newlen, :newlen-bgn_genp]

            res = axs[(i-3)*2+1, j].imshow(per_att,
                                       cmap=plt.cm.Blues,
                                       extent=[
                                           0, newlen-bgn_genp,
                                           newlen, bgn_genp,
            ...
            axs[(i-3)*2+1, j].set_xlabel('Attentions From', fontsize=fs)
            axs[(i-3)*2+1, j].set_ylabel('Attentions To', fontsize=fs)
            axs[(i-3)*2+1, j].title.set_text(
                'Generated Part\n Zoom in')
            axs[(i-3)*2+1, j].title.set_fontsize(fs)
        ...
        for j, (nl, nh) in enumerate(selected_layer_head_pairs):
            ...
            axs[(i-3)*2, j+4].set_xlabel('Attentions From', fontsize=fs)
            axs[(i-3)*2, j+4].set_ylabel('Attentions To', fontsize=fs)
            # plt.colorbar(res, ax=axs[i, j])
            axs[(i-3)*2, j+4].title.set_text(
                f'Neg. Cases {i+1}\n Layer {nl+1} Head {nh+1}')
            axs[(i-3)*2, j+4].title.set_fontsize(fs)

            per_att = per_att[bgn_genp:newlen, :newlen-bgn_genp]

            res = axs[(i-3)*2+1, 4+j].imshow(per_att,
                                         cmap=plt.cm.Blues,
                                         extent=[
                                             0, newlen-bgn_genp,
                                             newlen, bgn_genp,
            ...
            axs[(i-3)*2+1, 4+j].set_xlabel('Attentions From', fontsize=fs)
            axs[(i-3)*2+1, 4+j].set_ylabel('Attentions To', fontsize=fs)
            # axs[1, 4+i*2+j].title.set_text(
            # f'Neg. Cases {i+1} Local Zoom in\n of Layer {nl+1} Head {nh+1}')
            axs[(i-3)*2+1, 4+j].title.set_text(
                f'Generated Part\n Zoom in')
            axs[(i-3)*2+1, 4+j].title.set_fontsize(fs)

    plt.savefig(f"./attention_viz/visualization_14x8.pdf",
                pad_inches=0.1)
