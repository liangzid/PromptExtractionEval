...
def estimate_ppl():
    ...
    inps=llama_tokenizer(crime_train_example,return_tensors="pt").to("cuda:0")
    loss=base_model(**inps,labels=inps["input_ids"]).loss
    print(f"PPL of train sample: {loss}")

    inps1=llama_tokenizer(crime_test_example,return_tensors="pt").to("cuda:0")
...
def main_decaying_temp():
    ...
    for _ in range(5):
        ...
        ress.extend(res)
    res = ress
    with open("./temp_save_attack_decayingTemperature_samples222.json",
